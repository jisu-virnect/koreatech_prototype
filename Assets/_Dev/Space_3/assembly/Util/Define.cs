

using UnityEngine;

public static class Define
{
    public const string polygonGrid_Glow_Appear = nameof(polygonGrid_Glow_Appear);
    public const string polygonGrid_Glow_DisAppear = nameof(polygonGrid_Glow_DisAppear);
    public const string before = nameof(before);
    public const string trigger = nameof(trigger);
    public const string after = nameof(after);
    public const string world = nameof(world);

    public const string btn_370 = nameof(btn_370);
    public const string btn_370_disabled = nameof(btn_370_disabled);
    public const string btn_370_enabled = nameof(btn_370_enabled);
    public const string btn_370_finished = nameof(btn_370_finished);


    public const string check_Off = nameof(check_Off);
    public const string check_On = nameof(check_On);

    public static Color red = Color.red;
    public static Color blue = Color.blue;
    public static Color green = Color.green;
    public static Color white = Color.white;
    public static Color black = Color.black;

    public static Color32 blue_idle = new Color32(13, 24, 35, 153);
    public static Color32 green_success = new Color32(89, 179, 0, 153);

    public static Color32 color_black_0 = new Color32(0, 0, 0, 255);
    public static Color32 color_black_77 = new Color32(77, 77, 77, 255);
    public static Color32 color_black_153 = new Color32(153, 153, 153, 255);

}